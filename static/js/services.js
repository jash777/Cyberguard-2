document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateServicesTable() {
        ...
        servicesToShow.forEach((service) => {
            ...
            row.innerHTML = `
                ...
                <td>${escapeHtml(service.load)}</td>
                <td>${escapeHtml(service.active)}</td>
                <td>${escapeHtml(service.sub)}</td>
            `;
    ...
    function populateCategoryFilter(services) {
        const categories = [...new Set(services.map(service => service.load))];
        const fragment = document.createDocumentFragment();
    ...
    function showServiceDetails(service) {
        serviceMessage.innerHTML = `
            <strong>Unit:</strong> ${escapeHtml(service.unit)}<br>
            <strong>Description:</strong> ${escapeHtml(service.description)}<br>
            <strong>Load:</strong> ${escapeHtml(service.load)}<br>
            <strong>Active:</strong> ${escapeHtml(service.active)}<br>
            <strong>Sub:</strong> ${escapeHtml(service.sub)}
        `;
    ...
    function filterServices() {
        ...
        return services.filter(service => {
            const nameMatch = service.unit.toLowerCase().includes(searchTerm) || 
                              service.description.toLowerCase().includes(searchTerm);
            const categoryMatch = !selectedCategory || service.load === selectedCategory;
